...
class DatabaseService {
  ...
  async getStats(): Promise<{
    ...
    try {
      ...
      const byCategory = await this.allQuery("SELECT categories, COUNT(*) as count FROM plugins GROUP BY categories");
      const byManufacturer = await this.allQuery("SELECT manufacturer, COUNT(*) as count FROM plugins GROUP BY manufacturer");

      return {
        total: (total as any)?.count || 0,
        byCategory: Object.fromEntries((byCategory as any[]).map((row: any) => [row.categories, row.count])),
